...
float cLightHelper::calcApproxDistFromAtten(float targetLightLevel, float accuracy,
	...
	if (targetLightLevel != 0.0f) {
		if ((accuracy * 10.0f) >= targetLightLevel * 10.0f) {	// Adjust the accuracy by a hundreth
	...
	if (this->calcDiffuseFromAttenByDistance(cLightHelper::DEFAULTINFINITEDISTANCE,
		...
		accuracy) > targetLightLevelHigh) {	// Yes, so we can never get down to this light level
	...
	while (iterationCount < maxIterations) {
		...
		} else {	// Nailed it! Light level is within range, so return this distance
